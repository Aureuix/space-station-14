...
public sealed partial class AstroNavSystem : EntitySystem
{
    ...
    private void OnEquip(Entity<AstroNavComponent> ent, ref GotEquippedEvent args)
    {
        ...
        _alerts.ShowAlert(args.Equipee, ent.Comp.GPSAlert);
        EnsureComp<AstroNavMobComponent>(args.Equipee);
        
        // Do not override existing radar console (such as ghosts have)
        if (HasComp<RadarConsoleComponent>(args.Equipee))
            return;

        var radarComp = EnsureComp<RadarConsoleComponent>(args.Equipee);
    ...
    private void OnUnequip(Entity<AstroNavComponent> ent, ref GotUnequippedEvent args)
    {
        if(args.Slot != "id")
            return;
        _alerts.ClearAlert(args.Equipee, ent.Comp.GPSAlert);
        _uiSystem.CloseUi(args.Equipee, RadarConsoleUiKey.Key);
