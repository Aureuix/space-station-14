...
def __main__():
    ...
    print("Switching to SpectaLight branch...")
    switchBranch('SpectaLight')
    sleep(2)  # wait for branch switch to complete
    print("Running SpectaLight build...")
    devlines = runBuild()
    ...
    #convert to string
    if only_in_dev:
        diff_output += "Warnings only in SpectaLight build:\n"
        for line in only_in_dev:
            diff_output += line + "\n"

    if diff_output:
        print("Differences found between stable and SpectaLight builds:")
        print(diff_output)
        #output to a file
        ...
        print("No differences found between stable and SpectaLight builds.")
